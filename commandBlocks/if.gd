...
func get_highest_end_index():
	...
	for code in codingArea.get_children():
		if code.is_in_group("Command"):
			if code.commandName == "IfEnd" and code.get_index() > highestIndex:
				highestIndex = code.get_index()
	return highestIndex

# Called when the node enters the scene tree for the first time.
func _ready():
	tileType = $Background/TileDrop.get_item_text(0)
	tileLocation = $Background/LocationDrop.get_item_text(0)
	...
	if draggable == true:
		...
		if Input.is_action_pressed("click") and Global.is_dragging == true:
			...
			if frameDebounce == 0:
				...
				if dropZone != null:
					#print(dropZone.name)
					if commandName == "If" and $Background/IfScript.deployd == true and $Background/IfScript.endDeployd == true:
			...
			if get_closest_drop() != null:
				if get_closest_drop().name == "DeathArea":
					if commandName == "If":
						$Background/IfScript.ifEnd.ghostCode.queue_free()
						$Background/IfScript.ifEnd.queue_free()
						pass
...
func _on_background_mouse_entered():
	if Global.is_dragging == false and Global.state != "running":
		draggable = true
		if commandName != "If" or get_parent().name == "CodePicker":
